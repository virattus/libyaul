...
void __render_init(render_state_t* state)
{
	render_t* const render = state->render;
    workarea_mic3d_t * const workarea = state->workarea;
	...
	//TODO gonna have to do something about these, can't call them twice
...
void render_enable(const render_t* rndr, render_flags_t flags)
{
    rndr->render_flags |= flags;
}


void render_disable(const render_t* rndr, render_flags_t flags)
{
    rndr->render_flags &= ~flags;
}
...
inline void render_perspective_set(const render_t* rndr, angle_t fov_angle)
{
	rndr->view_distance = math3d_view_distance_calc(SCREEN_WIDTH, fov_angle);
}


inline void render_orthographic_set(const render_t* rndr, fix16_t scale)
{
	rndr->ortho_size = fix16_div(FIX16(SCREEN_WIDTH * SCREEN_RATIO), scale);
}

void render_near_level_set(const render_t* rndr, uint32_t level)
{
    const uint32_t clamped_level = clamp(level + 1, NEAR_LEVEL_MIN,
        NEAR_LEVEL_MAX);

    uint32_t near_value;
    near_value = rndr->view_distance;
    ...
    rndr->near = near_value;
}

void render_far_level_set(const render_t* rndr, uint32_t level)
{
    /*
     * 0: Origin
...
void render_sort_depth_set(const render_state_t* state, sort_list_t *sort_list_buffer, 
	uint16_t count)
{
...
void render_point_xform(const render_state_t* state, const fix16_mat43_t *world_matrix, 
...
void render_points_xform(const render_state_t* state, const fix16_mat43_t *world_matrix, 
...
void render_mesh_xform(const render_state_t* state, const mesh_t *mesh, const fix16_mat43_t *world_matrix)
{
	assert(state != NULL);
    assert(mesh != NULL);
    assert(world_matrix != NULL);
...
void render_cmdt_nocheck_insert(const render_state_t* state, const vdp1_cmdt_t *cmdt, fix16_t depth_z)
{
    assert(cmdt != NULL);
...
void render_cmdt_insert(const render_state_t* state, const vdp1_cmdt_t *cmdt, 
	fix16_t depth_z)
{
    assert(cmdt != NULL);
...
void render_cmdts_reserve(uint32_t cmdt_count __unused)
{
}


void render_cmdts_relinquish(void)
{
}


void render_end(render_state_t* state)
{
    render_t * const render = state->render;
    ...
    if (RENDER_FLAG_TEST(state, NO_CLEAR)) 
    {
...
void __render_single(const render_state_t* state, const sort_single_t *single)
{
    render_t * const render = state.render;
    sort_t * const sort = state.sort;
...
static void _screen_points_swap(int16_vec2_t *screen_points, uint32_t i, uint32_t j)
{
